...
use crate::{utils, val::Val, env::Env};
...
impl Expr {
    ...
    fn new_operations(s: &str) -> Result<(&str, Self), String> {
        let (s, lhs) = Self::parse(s)?;
        let (s, _) = utils::extract_whitespaces(s);

        let (s, op) = Op::parse(s)?;
        let (s, _) = utils::extract_whitespaces(s);

        let (s, rhs) = Self::parse(s)?;
    ...
    fn new_non_operation(s: &str) -> Result<(&str, Self), String> {
        Self::new_number(s)
            .or_else(|_| FuncCall::parse(s).map(|(s, func_call)| (s, Self::FuncCall(func_call))))
            .or_else(|_| {
...
mod tests {
    ...
    fn eval_binding_usage() {
        ...
        env.store_bindings("ten".to_string(), Val::Number(10));
        assert_eq!(
            Expr::BindingUsage(BindingUsage {
                name: "ten".to_string(),
            })
